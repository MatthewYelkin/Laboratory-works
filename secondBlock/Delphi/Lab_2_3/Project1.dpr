...
Begin
    Repeat
        IsCorrect := True;
        Write('Введите количество неизвестных системы уравнений: ');
        Try
...
Begin
    Repeat
        Writeln('Введите путь к файлу и его название.');
        IsCorrect := True;
        Readln(FilePath);
...
Begin
    ...
    Except
        Writeln('Ошибка ввода. Введите количество неизвестных c клавиатуры.');
...
begin
    If TakeChoice('Нажмите 0, если хотите считать информацию из консоли, 1 , если из файла.') = 0 then
...
Begin
    ...
    Writeln('Примечание : этот метод применим только для матриц, определитель которой отличен от нуля.');
End;
